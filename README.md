...
> Answer: The clint-side routing will notice when an event happens client side, and keep it from making another http:get request. It will then just move to a different component and alter the html address itself without making another request to show different content. This makes single page applications possible through routing.
